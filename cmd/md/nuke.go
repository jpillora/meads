...
func (c *nukeCmd) removeBeadsHook(hooksDir, name string, errors *[]string) {
	...
	// Hooks generated by beads carry a version marker near the top (e.g.
	// "# bd-hooks-version: 0.30.7") and consist entirely of beads logic — often
	// complex shell (heredocs, case blocks, var assignments) that the
	// line-by-line heuristic below fails to recognise, which would leave a live
	// hook behind. A marked hook is wholly beads, so delete the file outright.
	if isBeadsGeneratedHook(text) || isEntirelyBeadsHook(text) {
...
// isBeadsGeneratedHook reports whether a hook file was generated wholesale by
// beads, identified by the version marker beads stamps near the top of every
// hook it writes (e.g. "# bd-hooks-version: 0.30.7"). Such files contain only
// beads logic and can be removed in full, rather than relying on the
// line-by-line heuristics that miss complex generated shell (heredocs, case
// blocks, plain var assignments).
func isBeadsGeneratedHook(content string) bool {
	return strings.Contains(content, "bd-hooks-version")
}
