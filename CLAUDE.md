...
tasks file (`TASKS.md` or `TASKS.csv`) or git mode (tasks stored directly as
git refs, no working-tree file at all).

**This repo uses git mode.** There is no `TASKS.md` in the working tree — it
was migrated with `md convert TASKS.md --to-git` and deleted. Tasks live at
`refs/meads/tasks/<id>`, so they are not part of your commits and there is
nothing to stage: a task change lands the moment the command runs.

Everything below this line is generated by `md prime`, which always describes
whichever mode is actually active. If the mode ever changes again, regenerate
